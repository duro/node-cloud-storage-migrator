...
var Migrate = function(fromClient, toClient, options) {
  ...
  if (!toClient.provider) {
    this.toClient       = null;
    this.options        = toClient || {};
  } else {
  ...
  this.errors         = [];
  this.failedFiles    = [];
  this.showProgress   = this.options.cli;
...
// Kick off the migration process
Migrate.prototype.download = function(done) {
  async.waterfall([
    this.getFromContainer.bind(this),
    this.getFiles.bind(this)]
  , done);
    ...
    migrators.push(function(lastResult, done) {
      ...
      _this.fromClient.getFiles(container.name, options, function(err, files){
        ...
        if (_this.toClient) {
          ...
          _this.downloadFiles(files, {page: page, totalPages: numFilePages}, done);
        }
      })
    });
  };
...
Migrate.prototype.migrateFiles = function(files, options, next) {
  ...
  if (_this.showProgress && !areFailures) {
    ...
    if (_this.options.testMode && _this.options.testSize < index + 1) {
      ...
      fileQueue.push(file, function(err){
        ...
        if (_this.showProgress && !areFailures) {
          ...
          _this.logOut('Percent Complete: ' + Math.floor((completed / files.length) * 100) + '%')
        }
      });
      return true;
    }
...
Migrate.prototype.downloadFiles = function(files, options, next) {
  var fileQueue, progress,
      areFailures   = options.areFailures,
      completed     = 0,
      _this         = this,
  ...
  fileQueue = async.queue(function(file, done){
    ...
    if (file.contentType !== "application/directory") {
      ...
      if (_this.options.dryRun) {
        ...
        dlConfig.local = path.join(__dirname, '../', _this.fromClient.config.saveTo);
        _this.fromClient.download(dlConfig, function(err, downloadedFile){
            if (err) {
              _this.logOut('Error downloading: ' + file.name);
              _this.logOut(err);
      ...
      var dirToCreate = path.join(__dirname, '../', _this.fromClient.config.saveTo, file.name);
  ...
  if (options.page && options.totalPages) {
    console.log('Starting download (page %s of %s)...', options.page, options.totalPages);
  } else {
  ...
  fs.exists(path.join(__dirname, '../', _this.fromClient.config.saveTo), function(exists){
    if (!exists) {
      fs.mkdir(path.join(__dirname, '../', _this.fromClient.config.saveTo), function(err){
        if (err) return next(err);
        loadFilesIntoQueue();
      });
    } else {
...
Migrate.prototype.logOut = function(msg) {
  if (this.options.cli && this.options.logFile) {
